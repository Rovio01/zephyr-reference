...
def install_from_remote(package_config, dst_dir, remotes, default_remote):
    ...
    os.makedirs(dst_dir)
    vcs = NonRecursiveGitClient(fs.to_unix_path(dst_dir), remote_url, revision, True)
    assert vcs.export()
        ...
        for project_config in west_manifest.get("projects", []):
            ...
            if not os.path.isdir(package_path):
                ...
                if project_name == "trusted-firmware-m":
                    # Support for this module is not implemented at the moment
                    continue
                elif not install_from_registry(project_config, pm, package_path):
                    install_from_remote(
...
project_lib_deps = get_project_lib_deps(
    framework_modules_map,
    prebuilt_config,
)

# Note: `app` is replaced by files from the `src` folder.
# The `kernel`, `libopenthread` and `isr_tables` libs should
# be placed outside the `whole-archive` flags
weak_libs = []
whole_libs = []

for dep_id in project_lib_deps:
    lib_path = project_lib_deps[dep_id]["lib_path"]
    if dep_id.startswith(("kernel", "isr_tables", "app", "openthread")):
        weak_libs.append(lib_path)
    else:
...
# Default flags for project sources and linker
...
link_args["LINKFLAGS"] = filter_args(link_args["LINKFLAGS"], ["-"], ignore_flags)
...
# dependencies
env.Depends(preliminary_elf_path, [env.subst(library) for library in weak_libs])
...
env.Append(
    ...
    + " ".join([str(library) for library in weak_libs] + link_args["LIBS"]),
)

# Note: Standard libraries in LIBS are already added to the LINKCOMMAND in _LIBFLAGS
# Explicitly append build and link flags instead of env.MergeFlags
# to preserve flags order
del link_args["LIBS"]
env.Append(**build_flags)
env.Append(**link_args)
